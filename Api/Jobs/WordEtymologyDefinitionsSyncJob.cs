...
namespace Api.Jobs
{
    public class WordEtymologyDefinitionsSyncJob(
        WordEtymologyDefinitionsSyncService syncService,
        ISimpleCache simpleCache,
        IOptions<WordEtymologyDefinitionsSyncConfig> config,
        ILogger<WordEtymologyDefinitionsSyncJob> logger)
    {
        private readonly WordEtymologyDefinitionsSyncService _syncService = syncService;
        private readonly ISimpleCache _simpleCache = simpleCache;
        private readonly WordEtymologyDefinitionsSyncConfig _config = config.Value;
        private readonly ILogger<WordEtymologyDefinitionsSyncJob> _logger = logger;
        ...
        public async Task ExecuteAsync()
        {
            ...
            if (checkpointPage > 0)
            {
                _logger.LogInformation(
                    "WordEtymologyDefinitionsSync.JobResumedFromCheckpoint runId={RunId} startPage={StartPage} checkpointPage={CheckpointPage} pageSize={PageSize}",
                    runId,
                    startPage,
                    checkpointPage,
                    _config.PageSize);
            ...
            {
                _logger.LogInformation(
                    "WordEtymologyDefinitionsSync.JobStarted runId={RunId} startPage={StartPage} pageSize={PageSize}",
                    runId,
                    startPage,
                    _config.PageSize);
            }
            ...
            try
            {
                ...
                for (var page = startPage; page < startPage + _config.MaxPagesPerRun; page++)
                {
                    ...
                    _logger.LogInformation(
                        "WordEtymologyDefinitionsSync.PageStarted runId={RunId} currentPage={CurrentPage} pageSize={PageSize}",
                        runId,
                        page,
                        _config.PageSize);
                    ...
                    if (pageResult.WordsRead == 0)
                    {
                        _logger.LogInformation(
                            "WordEtymologyDefinitionsSync.JobCompleted runId={RunId} reason=no-more-records processedPages={ProcessedPages} totalDurationMs={TotalDurationMs}",
                            runId,
                            processedPages - 1,
                    ...
                    _logger.LogInformation(
                        "WordEtymologyDefinitionsSync.PageCompleted runId={RunId} currentPage={CurrentPage} wordsRead={WordsRead} etymologyRowsRead={EtymologyRowsRead} distinctParts={DistinctParts} distinctPartMeaningPairs={DistinctPartMeaningPairs} wordsCreated={WordsCreated} definitionsAdded={DefinitionsAdded} duplicatesSkipped={DuplicatesSkipped} partiallyCreated={PartiallyCreated} noValidDefinitions={NoValidDefinitions} pageDurationMs={PageDurationMs}",
                        runId,
                        pageResult.Page,
                        pageResult.WordsRead,
                    ...
                    _logger.LogInformation(
                        "WordEtymologyDefinitionsSync.CheckpointSaved runId={RunId} currentPage={CurrentPage} checkpointWritten=true",
                        runId,
                        pageResult.Page);

                    if (pageResult.WordsRead < _config.PageSize)
                    {
                        _logger.LogInformation(
                            "WordEtymologyDefinitionsSync.JobCompleted runId={RunId} reason=last-page-detected processedPages={ProcessedPages} totalDurationMs={TotalDurationMs}",
                            runId,
                            processedPages,
                            overallStopwatch.ElapsedMilliseconds);
                        await _simpleCache.SetAsync(LastRunStatusKey, "completed");
                ...
                _logger.LogWarning(
                    "WordEtymologyDefinitionsSync.JobCompleted runId={RunId} reason=max-pages-cap-reached maxPagesPerRun={MaxPagesPerRun} totalDurationMs={TotalDurationMs}",
                    runId,
            ...
            catch (Exception ex)
            {
                ...
                _logger.LogError(
                    ex,
                    "WordEtymologyDefinitionsSync.JobFailed runId={RunId} startPage={StartPage} failureType={FailureType} exceptionMessage={ExceptionMessage} totalDurationMs={TotalDurationMs}",
                    runId,
                    startPage,
                    ex.GetType().Name,
                    ex.Message,
