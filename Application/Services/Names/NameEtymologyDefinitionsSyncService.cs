...
{
    public record NameEtymologyDefinitionsSyncPageResult(
        int Page,
        int NamesRead,
    ...
    public class NameEtymologyDefinitionsSyncService(
        INameEntryRepository nameEntryRepository,
        WordEntryService wordEntryService,
        ILogger<NameEtymologyDefinitionsSyncService> logger)
    {
        ...
        private readonly ILogger<NameEtymologyDefinitionsSyncService> _logger = logger;

        public async Task<NameEtymologyDefinitionsSyncPageResult> ProcessPageAsync(
            int page,
            int pageSize,
            string runId,
        ...
        {
            var names = await _nameEntryRepository.GetPublishedWithEtymologyPageAsync(page, pageSize);

            var flattenedEtymology = names
                .SelectMany(name => name.Etymology ?? [])
                ...
                .Take(Math.Max(0, wordLogSampleSize)))
            {
                _logger.LogInformation(
                    "NameEtymologyDefinitionsSync.WordResult runId={RunId} page={Page} word={Word} status={Status}",
                    runId,
                    page,
                    sampledWord.Key,
                    sampledWord.Value);
            ...
            return new NameEtymologyDefinitionsSyncPageResult(
                page,
                names.Count,
